...
export const getBackupPathsDetails = async (): Promise<BackupPathDetails[]> => {
    const { data } = await api.get('/crud/rutas-respaldo/details');
    return z.array(BackupPathDetailsSchema).parse(data);
};
